...
export class Carts {
  ...
  static db = read(Carts.pathCarts)
  ...
  static delete = async (req, res) => {
    try {
      ...
      await save(Carts.pathCarts, cartsArray)
  ...
  static addProductOnCart = async (req, res) => {
    try {
      ...
      let cart = cartsArray.find((cart) => cart.id == req.params.cid)
      product.find((product) => product.id == req.body.id)
        ? res.send(cart.products.push(req.body))
  ...
  static deleteProduct = async (req, res) => {
    ...
    if (cart) {
      ...
      if (product) {
        ...
        await save(Carts.pathCarts, Carts.db)
        res.status(200).json(cart)
      } else {
        res.status(404).json({
